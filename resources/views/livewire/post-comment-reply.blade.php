<div class="flex">
    <div class="flex-shrink-0 mr-3">
        <img class="mt-3 rounded-full w-6 h-6 sm:w-8 sm:h-8" src="{{$reply->user->profile_photo_url}}" alt="">
    </div>
    <div class="flex-1 rounded-lg px-4 py-2 space-y-2 sm:px-6 sm:py-4 leading-relaxed w-full">
        <strong>{{$reply->user->name}}</strong>
        <span class="text-xs text-gray-400 ml-1">{{$reply->created_at->diffForHumans(now())}}</span>
        ...
        <div class="flex items-center justify-start space-x-2">
            <div class="text-sm text-gray-500 font-semibold">
                <a href="javascript:;" class="text-blue-500" wire:click="$emitTo('post-comment','replyToComment:{{$reply->post_comment_id}}','{{$reply->user_id}}')">回复</a>
            </div>
            ...
            <div class="cursor-pointer" wire:click="$emitUp('showModal',{{$reply->id}},'{{$reply->content}}','post-comment-reply')">
                <div class="flex justify-center items-center flex-col py-2">
